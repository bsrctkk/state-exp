...
export default function SupplierTable() {
    ...
    const [supliers, setsupliers] =useState(supplierData)

    const search = (value) => {
        let filteredSuplier = supplierData.filter(x => x.companyName.toLowerCase().includes(value.toLowerCase()));
        setsupliers([...filteredSuplier])
    }

    const [Ascending, setAscending] = useState(true);
    
      const sort = () => {
        const sortedSuplier = [...supplierData].sort();
        if (!Ascending) {
          sortedSuplier.reverse();
        }
        setsupliers(sortedSuplier);
  ...
  return (<>
    ...
    <div>
    <h1> Lenght: {supliers.length}</h1>
    </div>
    <table className='w3-table' style={tableStyle}>
        <thead>
            <tr>
                <th style={thStyle}>Id</th>
                <th onClick={sort} style={thStyle}>CompanyName</th>
                <th style={thStyle}>ContactName</th>
                <th style={thStyle}>ContactTitle</th>
            </tr>
        </thead>
        <tbody>
            {
                supliers.map(item => {
                    return <tr>
                        ...
                        <td style={tdStyle}>{item.contactTitle}</td>
                    </tr>
                })
            }
        </tbody>
